...
export default function UsarModeloClient({ templateId }: UsarModeloClientProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch(`/api/modelos/${templateId}/use`);
  ...
  useEffect(() => {
    ...
    // Inicializar itens da empresa selecionada
    if (comp.items?.length) {
      const itemState: Record<string, { selected: boolean; quantity: number }> = {};
      comp.items.forEach((item: { id: string; quantity: number }) => {
        itemState[item.id] = { selected: true, quantity: item.quantity };
      });
  ...
  // Atualizar variáveis de itens quando seleção muda
  useEffect(() => {
    if (!selectedCompany) return;
    const comp = companies.find(c => c.id === selectedCompany);
    if (!comp?.items?.length) return;

    const activeItems = comp.items.filter((item: { id: string }) => selectedItems[item.id]?.selected);
    if (activeItems.length === 0) return;

    const formatCurrency = (v: number) => new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(v);

    // Calcular valor total de todos os itens selecionados
    const valorTotal = activeItems.reduce((sum: number, item: { id: string; unitPrice: number }) => {
      const qty = selectedItems[item.id]?.quantity ?? 0;
      return sum + (qty * item.unitPrice);
    }, 0);

    // Gerar tabela HTML de itens formatada
    const itensHtml = `<table style="width:100%;border-collapse:collapse;font-size:12px;margin:8px 0">
<thead><tr style="background:#f3f4f6">
<th style="border:1px solid #d1d5db;padding:6px;text-align:center">Item</th>
<th style="border:1px solid #d1d5db;padding:6px;text-align:left">Descrição</th>
<th style="border:1px solid #d1d5db;padding:6px;text-align:center">Unid.</th>
<th style="border:1px solid #d1d5db;padding:6px;text-align:center">Qtd.</th>
<th style="border:1px solid #d1d5db;padding:6px;text-align:right">Valor Unit.</th>
<th style="border:1px solid #d1d5db;padding:6px;text-align:right">Total</th>
</tr></thead><tbody>${activeItems.map((item: { name: string; description?: string; unit: string; unitPrice: number; id: string }, i: number) => {
      const qty = selectedItems[item.id]?.quantity ?? 1;
      const total = qty * item.unitPrice;
      return `<tr>
<td style="border:1px solid #d1d5db;padding:6px;text-align:center">${String(i + 1).padStart(2, "0")}</td>
<td style="border:1px solid #d1d5db;padding:6px">${item.name}${item.description ? ` — ${item.description}` : ""}</td>
<td style="border:1px solid #d1d5db;padding:6px;text-align:center">${item.unit}</td>
<td style="border:1px solid #d1d5db;padding:6px;text-align:center">${qty}</td>
<td style="border:1px solid #d1d5db;padding:6px;text-align:right">${formatCurrency(item.unitPrice)}</td>
<td style="border:1px solid #d1d5db;padding:6px;text-align:right">${formatCurrency(total)}</td>
</tr>`;
    }).join("")}</tbody>
<tfoot><tr style="background:#f3f4f6;font-weight:bold">
<td colspan="5" style="border:1px solid #d1d5db;padding:6px;text-align:right">VALOR TOTAL:</td>
<td style="border:1px solid #d1d5db;padding:6px;text-align:right">${formatCurrency(valorTotal)}</td>
</tr></tfoot></table>`;

    // Gerar texto simples de itens (fallback)
    const itensTexto = activeItems
      .map((item: { name: string; unit: string; unitPrice: number; id: string }, i: number) => {
        const qty = selectedItems[item.id]?.quantity ?? 1;
        const total = qty * item.unitPrice;
        return `${i + 1}. ${item.name} — ${qty} ${item.unit} x ${formatCurrency(item.unitPrice)} = ${formatCurrency(total)}`;
      })
      .join("\n") + `\nVALOR TOTAL: ${formatCurrency(valorTotal)}`;

    setVariables(prev => {
      const updated = { ...prev };
      const itemVarMap: Record<string, string> = {
        // Variáveis de tabela HTML (para {e_itens} e similares)
        e_itens: itensHtml,
        itens_contratados: itensHtml,
        itens: itensHtml,
        itens_fornecidos: itensHtml,
        descricao_itens: itensHtml,
        tabela_itens: itensHtml,
        // Variáveis de texto simples
        lista_itens: itensTexto,
        // Variáveis de valor total
        e_valor_total: formatCurrency(valorTotal),
        valor_total_itens: formatCurrency(valorTotal),
        valor_total: formatCurrency(valorTotal),
        valor_contrato: formatCurrency(valorTotal),
        valor_global: formatCurrency(valorTotal),
        valor_requerente: formatCurrency(valorTotal),
        // Quantidade de itens
        qtd_itens: String(activeItems.length),
        quantidade_itens: String(activeItems.length),
      };
      Object.entries(itemVarMap).forEach(([varName, value]) => {
  ...
  }, [selectedItems, selectedCompany, companies]);
    ...
    try {
      ...
      const res = await fetch(`/api/modelos/${templateId}/use`, {
        ...
        body: JSON.stringify({
          ...
          companyItems: selectedCompany ? Object.entries(selectedItems)
            .filter(([, v]) => v.selected)
            .map(([id, v]) => {
              const comp = companies.find(c => c.id === selectedCompany);
              const item = comp?.items?.find((i: { id: string }) => i.id === id);
              return item ? { id, name: item.name, description: item.description, unit: item.unit, quantity: v.quantity, unitPrice: item.unitPrice } : null;
            })
            .filter(Boolean) : [],
  ...
  if (!template) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/modelos" className="text-[#1E3A5F] hover:underline mt-2 inline-block">Voltar aos modelos</Link>
      </div>
    );
  }
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href="/modelos" className="p-2 hover:bg-gray-100 rounded-lg transition-colors">
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="bg-gradient-to-br from-blue-50/80 to-indigo-50/80 rounded-2xl shadow-sm border border-blue-100 p-5 space-y-3">
            ...
            {selectedCompany && (() => {
              ...
              const formatCurrency = (v: number) => new Intl.NumberFormat("pt-BR", { style: "currency", currency: "BRL" }).format(v);
              const activeItems = comp.items.filter((item: { id: string }) => selectedItems[item.id]?.selected);
              const totalGeral = activeItems.reduce((sum: number, item: { id: string; unitPrice: number }) => {
                const qty = selectedItems[item.id]?.quantity ?? 0;
                return sum + (qty * item.unitPrice);
              }, 0);
              return (
                <div className="bg-white/70 rounded-xl p-3 border border-emerald-200/80">
                  <label className="text-xs font-medium text-gray-600 mb-2 flex items-center gap-1.5">
                    <Package className="w-3.5 h-3.5 text-emerald-600" /> Itens do Contrato
                    <span className="text-[10px] text-emerald-600 bg-emerald-100 px-1.5 py-0.5 rounded-full ml-auto">
                      {activeItems.length}/{comp.items.length} selecionados
                    </span>
                  </label>
                  <div className="space-y-1.5 max-h-[200px] overflow-y-auto pr-1">
                    {comp.items.map((item: { id: string; name: string; description?: string; unit: string; quantity: number; unitPrice: number }) => {
                      const isSelected = selectedItems[item.id]?.selected ?? false;
                      const qty = selectedItems[item.id]?.quantity ?? item.quantity;
                      return (
                        <div key={item.id} className={`rounded-lg border p-2 transition-all ${isSelected ? "border-emerald-300 bg-emerald-50/50" : "border-gray-200 bg-white/50 opacity-60"}`}>
                          <label className="flex items-start gap-2 cursor-pointer">
                            <input
                              ...
                              className="w-3.5 h-3.5 text-emerald-500 rounded border-gray-300 mt-0.5"
                            />
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-1">
                                <input
                                  ...
                                  className="w-16 px-1.5 py-0.5 border border-gray-200 rounded text-[11px] text-center disabled:opacity-40"
                                />
                                <span className="text-[10px] text-gray-500">{item.unit} × {formatCurrency(item.unitPrice)}</span>
                                {isSelected && (
                  ...
                  {activeItems.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-emerald-200 flex justify-between items-center">
                      <span className="text-[10px] text-gray-600">Total dos itens:</span>
                      <span className="text-xs font-bold text-emerald-700">{formatCurrency(totalGeral)}</span>
                    </div>
                  )}
                </div>
              );
