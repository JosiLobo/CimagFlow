...
export default function TemplatesClient() {
  ...
  const handleSave = async () => {
    ...
    try {
      const url = editId ? `/api/modelos/${editId}` : "/api/modelos";
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleting(id);
    try { await fetch(`/api/modelos/${id}`, { method: "DELETE" }); fetchTemplates(); }
  ...
  const savePreviewChanges = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/modelos/${showPreview.id}`, {
        method: "PATCH",
