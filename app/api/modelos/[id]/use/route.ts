    ...
    const doc = await prisma.document.create({
      data: {
        ...
        items: companyItems?.length ? {
          create: companyItems.map((item: { name: string; description?: string; unit: string; quantity: number; unitPrice: number }) => ({
            name: item.name,
      ...
      include: { signers: { include: { signer: true } }, items: true },
    });
